...
async function get_last_number(topic_id) {
    console.log(`Fetching last number for topic_id: ${topic_id}`);
    ...
    const response = await fetch("https://artofproblemsolving.com/m/community/ajax.php", {
        ...
        body: `topic_id=${topic_id}&direction=backwards&start_post_id=-1&start_post_num=9999999&show_from_time=-1&num_to_fetch=1&a=fetch_posts_for_topic&aops_logged_in=true&aops_user_id=${AoPS.session.id}&aops_session_id=${AoPS.session.id}`,
        method: "POST",
        mode: "cors",
        credentials: "include"
    });
    ...
    return data.response.posts[0].post_number
}
